...
"""The registry's Python accessors: registry(), describe(), resolve(), defaults(), compose(), api_index().

One module-agnostic file, embedded with SWIG's ``%pythoncode`` into every extension module whose C++
side registers through ``RegistryCore.h`` and exports ``registry_json()``: tttrlib's
(``ext/python/Registry.i``) and imp.bff's, which vendors this file and pins it. The functions act on the
module they were embedded into, so ``tttrlib.describe(name)`` and ``IMP.bff.describe(name)`` are the same
code reading different tables -- one mechanism, and a consumer such as ChiSurf reads both the same way.
Moved out of Registry.i on 2026-09-15 (imp.bff PRD-147 amendment A2); the examples use tttrlib.
"""


def _registry_module():
    """The extension module this file was embedded into."""
    import sys as _sys
