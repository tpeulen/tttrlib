...
- **Opt-in "historic MaxEnt": find the entropy weight automatically by
  ...
  fixed-nu path; >0 finds nu via a joint (p, nu) Gull-Skilling controller —
  nu is updated *inside* the MEM loop by a secant in log-nu/log-chisq space,
  warm-started, and the caller's `nu` seeds the search — so the fit's
  chi-square lands at the target), and `decay_pattern_fit` gains
  `PatternFitMode_kMaxEntTargetChisq` (there, `reg_strength` *is* the
  target). Results report `nu_used` and a converged flag; unreachable
  targets degrade gracefully to the closest endpoint instead of crashing.
  The search is not an outer bisection for a measured reason: on a steep
  1M-photon FRET case (100-point distance grid) an outer root-find
  cold-starting the MEM solve once per nu probe exhausted 500 solves
  (~143 s) without reaching the target, while the joint controller converges
  to chi²=1.0000 in 157 warm-started QP steps (~4 s); that fixture is now a
  regression test. Two unit conventions, easy to conflate: the TCSPC
  solvers' chi-square is a MEAN over fit bins (classic target ~1.0),
  `decay_pattern_fit`'s is the raw sum (~n_bins). Built test-first: a Python
  prototype on the existing `tcspc_run_mem` binding was validated —
  including against decays generated by the photon simulator (`SimEngine`),
  not just analytic curves — before the C++ engine (`run_mem_target_chisq`,
  MaxEntQp.h) was written, and the prototype stays in
  `test_maxent_tcspc.py` as the reference the C++ must agree with (asserted
  to 1e-6 on nu). Deliberately opt-in and never a default: chisurf, the
  reference implementation, argues target-chi² nu selection is statistically
  naive for decay data and ships only an L-curve *suggestion* — see PRD-039
